    ...
    <el-table
     ...
     <!--  <el-table-column
      ...
      <el-table-column
        label="报价截止日期">
        <template slot-scope="scope">
          <span v-if="scope.row.endDate">{{ parseTime(scope.row.endDate,'{y}-{m}-{d} {h}:{i}:{s}')}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="状态">
...
import { enquiryInfoList } from '@/api/enquiryQuotationInfo'
...
export default {
  ...
  data() {
    return {
      ...
      total:0,
      dialogbtn:false,
      statuslist:[{id:0,value:'询价中'},{id:1,value:'已完成'},{id:2,value:'已关闭'}],
      searchDate:[],
      pickerOptions: {
        shortcuts: [{
          text: '最近一周',
  ...
  methods: {
    parseTime,
    getlist() {
      enquiryInfoList({pageNum:this.current,pageSize:this.pageSize,...this.planform}).then(
        res => {
