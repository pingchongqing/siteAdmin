...
<div class="app-container">
  <div class="formcontain" v-loading="loading">
    <el-form :model="planform" ref="ruleForm" label-width="120px">
      ...
      <el-row :gutter="10">
        <!-- <el-col :span="6">
          <span class="texttitle">{{'询价标题:'+(planform.ticketNo?planform.ticketNo:'')}}</span>
        </el-col> -->
        <!-- <el-col :span="6">
           <span class="texttitle">{{'报价截止日期:'+(planform.endDate?parseTime(planform.endDate,'{y}-{m}-{d} {h}:{i}:{s}'):'')}}</span>
        </el-col>
        <el-col :span="6">
          <span class="texttitle">{{'报价企业(总数):'+(data?(data.length):'')}}</span>
        </el-col>
        <el-col :span="6">
          ...
          <el-row :gutter="30" >
            ...
            <el-col :span="8">
              <el-form-item label="供货区域:">
                <span>{{(planform.provinceName?planform.provinceName:'')+(planform.cityName?planform.cityName:'')}}</span>
              </el-form-item>
            </el-col>
            <el-col :span="8">
...
<script>
import { enquiryInfoDetail } from '@/api/enquiryQuotationInfo'
...
export default {
  ...
  methods: {
    ...
    getdetail(){
      enquiryInfoDetail({orderNo:this.$route.params.orderNo}).then(res=>{
